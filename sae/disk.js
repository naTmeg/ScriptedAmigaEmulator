...
function Drive(number) {
	...
	this.trackdata = new Array(MAX_TRACKS); for (var i = 0; i < MAX_TRACKS; i++) this.trackdata[i] = new trackid();
	//this.writetrackdata = new Array(MAX_TRACKS);	for (var i = 0; i < MAX_TRACKS; i++) this.trackdata[i] = new trackid();
	...
	this.insert = function() {
		...
		if (strncmp_as(this.diskfile, 'UAE-1ADF', 8) == 0) {
			...
			for (var i = 0; i < this.num_tracks; i++) {
				...
				this.trackdata[i].revolutions = 1;
	...
	this.set_motor = function(off) {
		...
		if (!this.motoroff && off) {
			this.drive_id_scnt = 0; /* Reset id shift reg counter */
	...
	this.decode_amigados = function() {
		...
		for (sec = 0; sec < this.num_secs; sec++) {
			...
			for (i = 0; i < 544; i++) {
				this.bigmfmbuf[bigmfmpos % len] = mfmbuf[i]; bigmfmpos++;
			}
		}
	}
	...
	this.fill_bigbuf = function(force) {
		...
		/*if (this.writediskfile && this.writetrackdata[tr].bitlen > 0) {
			var i;
			trackid *wti = &this.writetrackdata[tr];
			this.tracklen = wti->bitlen;
			this.revolutions = wti->revolutions;
...
function Disk() {
			...
			if (!(selected & (1 << dr)) && (prev_selected & (1 << dr)) ) {
				...
				if (!(disabled & (1 << dr))) {
					if ((prev_data & 0x80) == 0 || (data & 0x80) == 0)
						...
						floppy[dr].set_motor(1); /* motor on: if motor bit = 1 in prevdata only (motor flag state in data has no effect) -> turn motor off */
				}
				if (dr == 0)
					floppy[dr].idbit = 0;
			}
	...
	/*static this.read_floppy_data (struct zfile *diskfile, trackid *tid, var offset, var *dst, var len) {
		if (len == 0)
			return;
			...
			for (dr = 0; dr < MAX_FLOPPY_DRIVES; dr++) {
				...
				if (dskdmaen == DSKDMA_READ) { //TURBO read
					...
					for (i = 0; i < dsklength; i++) {
						var w = AMIGA.mem.load16(dskpt + i * 2);
						floppy[dr].bigmfmbuf[pos >> 4] = w;
						pos += 16;
						pos %= floppy[dr].tracklen;
					}
					floppy[dr].write_data();
